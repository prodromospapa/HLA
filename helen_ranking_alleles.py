        ...
        for ranking in ranking_list:
                ...
                if plot:
                        if sort_way == 0:
                                ...
                                plt.legend(["0","1","2"])
                                plt.tight_layout()
                                plt.savefig(f"{args.choose}_{args.direction}_ranking_{ranking[0]}_{ranking[1]}_sortbyname.png",dpi=300)
                                plt.close()
                        else:
                                plt.plot([x[0] for x in zeros],[x[1] for x in zeros],color="blue")
                                ...
                                plt.xticks(rotation=90)
                                plt.tight_layout()
                                plt.savefig(f"{args.choose}_{args.direction}_ranking_{ranking[0]}_{ranking[1]}_zeros_sortbyvalue.png",dpi=300)
                                plt.close()
                                ...
                                plt.xticks(rotation=90)
                                plt.tight_layout()
                                plt.savefig(f"{args.choose}_{args.direction}_ranking_{ranking[0]}_{ranking[1]}_ones_sortbyvalue.png",dpi=300)
                                plt.close()
                                ...
                                plt.xticks(rotation=90)
                                plt.tight_layout()
                                plt.savefig(f"{args.choose}_{args.direction}_ranking_{ranking[0]}_{ranking[1]}_twos_sortbyvalue.png",dpi=300)
                                plt.close()
                else:
