...
def ranking_plot(plot,ranking_list,sort):
        for ranking in ranking_list:
                #alleles
                ranking_values_list = [[x[0]+"_"+base.name,x[1]] for index,base in mm_per[freq_per<ranking[0]][freq_per >=ranking[1]].iterrows() if base.dropna().values.all() for x in base.dropna().items()]
                ...
                #total
                ranking_values_list_total = [y for x in mm_per[freq_per<ranking[0]][freq_per >=ranking[1]].values.tolist() for y in x if type(y).__name__=="dict"]
