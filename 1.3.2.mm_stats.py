...
def counter_genotype(data):
        ...
        for loci in ["A","B","C","DRB1","DQB1"]:
                if args.type == "allele":
                        ...
                        loci_list=["_".join(i) for i in data[loci].tolist()]
...
freq_per = counter_genotype(genotypes)

mm_per_df = pd.DataFrame(mm_per)
freq_per_df = pd.DataFrame(freq_per).reindex(mm_per_df.index)

mm_per_df.to_pickle(f"{args.choose}_{args.direction}_{args.type}_mm_per.pickle")
freq_per_df.to_pickle(f"{args.type}_freq_per.pickle")
